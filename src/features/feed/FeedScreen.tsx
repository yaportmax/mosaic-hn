...
export function FeedScreen() {
  ...
  const action = useCallback(async (storyId: number, gesture: GestureAction) => {
    ...
    if (!story) return;
    await performStoryAction(gesture, story, database.repository, preferences, theme.tokens.motion.durationScale === 0);
    await data.reloadFromCache();
  }, [data, database.repository, preferences, theme.tokens.motion.durationScale]);
  ...
  const subtitle = useMemo(() => data.lastUpdated ? `${data.preset.name} · updated ${formatRelativeTime(data.lastUpdated)} ago${data.hiddenCount ? ` · ${data.hiddenCount} hidden` : ''}` : `${data.preset.name} local ranking`, [data.lastUpdated, data.preset.name, data.hiddenCount]);

  return <Screen edges={['top']}>
    <ScreenHeader title="Mosaic HN" subtitle={subtitle} actions={<IconButton icon="options-outline" label="Feed presets" onPress={() => router.push('/presets')} />} />

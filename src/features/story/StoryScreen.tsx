...
export function StoryScreen({ id }: { id: number }) {
  ...
  if (data.loading && !story) return <Screen edges={['top']}><DetailHeader title="Story" /><LoadingState label="Loading story and discussion…" /></Screen>;
  ...
  const addToCollection = async () => {
    ...
    if (!collections.length) { Alert.alert('No collections', 'Create a collection from the Library tab first.'); return; }
  ...
  const summaryContent = <View style={[
    ...
    <View style={[styles.meta, storyLayout === 'line' && styles.metaCompact]}><Pressable onPress={() => router.push({ pathname: '/user/[id]', params: { id: story.by } })}><ThemedText variant="meta" accent>by {story.by}</ThemedText></Pressable><ThemedText variant="meta" muted>{formatRelativeTime(story.time)}</ThemedText><ThemedText variant="meta" muted>{formatNumber(story.score, preferences.compactNumbers)} points</ThemedText><ThemedText variant="meta" muted>{formatNumber(story.descendants, preferences.compactNumbers)} comments</ThemedText>{story.domain ? <Pressable onPress={() => router.push({ pathname: '/discovery/domain/[domain]', params: { domain: story.domain! } })}><ThemedText variant="meta" muted>{story.domain}</ThemedText></Pressable> : null}</View>
  ...
  const header = <View style={styles.storyHeader}>
    ...
    <Section title="Local timeline" caption="Snapshots are captured only when this installation sees the story."><Surface style={styles.sectionSurface}><TimelineChart snapshots={data.timeline} /></Surface></Section>
    <Section title="Library notes" caption="Stored only on this device.">
      ...
      {notesOpen ? <Surface style={styles.notes}><TextInput value={note} onChangeText={setNote} placeholder="Your notes…" placeholderTextColor={theme.tokens.colors.mutedText} multiline style={[styles.noteInput, { color: theme.tokens.colors.text, borderColor: theme.tokens.colors.border }]} /><TextInput value={tags} onChangeText={setTags} placeholder="tags, separated, by commas" placeholderTextColor={theme.tokens.colors.mutedText} autoCapitalize="none" style={[styles.tagInput, { color: theme.tokens.colors.text, borderColor: theme.tokens.colors.border }]} /><Button label="Save locally" onPress={() => void Promise.all([data.saveNote(note), data.saveTags(tags.split(',').map((value) => value.trim()).filter(Boolean))])} /></Surface> : null}
    </Section>
    {data.related.length ? <Section title="Related in your archive" caption="Local deterministic title and text similarity."><View style={styles.related}>{data.related.map(({ story: related, similarity }) => <Pressable key={related.id} onPress={() => router.push({ pathname: '/story/[id]', params: { id: String(related.id) } })} style={({ pressed }) => ({ opacity: pressed ? 0.65 : 1 })}><Surface style={styles.relatedCard}><ThemedText variant="headline">{related.title}</ThemedText><ThemedText variant="caption" muted>{related.domain ?? 'HN'} · similarity {Math.round(similarity * 100)}%</ThemedText></Surface></Pressable>)}</View></Section> : null}
    <View style={styles.commentsTitle}><ThemedText variant="title">Discussion</ThemedText>{data.commentsLoading ? <ThemedText variant="meta" accent>Loading branches…</ThemedText> : <ThemedText variant="meta" muted>{data.rows.length} cached comments</ThemedText>}</View>
    <HorizontalControls><Chip compact label="New" onPress={() => jump('new')} /><Chip compact label="OP" onPress={() => jump('op')} /><Chip compact label="Saved" onPress={() => jump('saved')} /><Chip compact label="Large threads" onPress={() => jump('large')} /></HorizontalControls>
  </View>;
  ...
  return <Screen edges={['top']}>
    ...
    <View style={styles.listWrap}>
      <FlashList ref={listRef} data={data.rows} keyExtractor={(row) => String(row.comment.id)} renderItem={({ item }) => <CommentRowView row={item} onToggle={() => data.toggleCollapsed(item.comment.id)} onSave={() => void data.toggleSavedComment(item.comment.id)} />} ListHeaderComponent={header} ListEmptyComponent={<EmptyState icon="chatbubble-outline" title="No comments" body={data.commentsLoading ? 'The discussion is still loading.' : 'This story has no cached comments yet.'} />} ListFooterComponent={data.commentsLoading ? <LoadingState label="Loading more comment branches…" /> : <View style={{ height: 100 }} />} drawDistance={1_000} />
      <ThreadMinimap rows={data.rows} onSelect={(index) => listRef.current?.scrollToIndex({ index, animated: true, viewPosition: 0.1 })} />
    </View>
  </Screen>;
}
const styles = StyleSheet.create({ listWrap: { flex: 1 }, storyHeader: { gap: 18, paddingHorizontal: 14, paddingTop: 12 }, summary: { gap: 12 }, summaryLine: { gap: 7 }, summaryCard: { padding: 18 }, summaryEditorial: { gap: 16, paddingVertical: 20, borderBottomWidth: StyleSheet.hairlineWidth }, meta: { flexDirection: 'row', flexWrap: 'wrap', gap: 9 }, metaCompact: { gap: 6 }, actions: { gap: 8, paddingRight: 16 }, storyText: { padding: 16 }, storyTextLine: { paddingVertical: 10 }, sectionSurface: { padding: 14 }, notes: { padding: 12, gap: 10 }, noteInput: { minHeight: 120, textAlignVertical: 'top', borderWidth: 1, borderRadius: 10, padding: 12, fontSize: 15 }, tagInput: { minHeight: 44, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, fontSize: 15 }, related: { gap: 8 }, relatedCard: { padding: 13, gap: 4 }, commentsTitle: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', gap: 10, marginTop: 4 } });

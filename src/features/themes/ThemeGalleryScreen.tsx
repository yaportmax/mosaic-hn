...
export function ThemeGalleryScreen() {
  ...
  return <Screen edges={['top']}>
    ...
    {loading ? <LoadingState label="Loading theme catalog…" /> : <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadCatalog()} />} contentContainerStyle={styles.content}>
      <Section title="Installed" caption={`${catalog.length} validated theme packages. Tap to inspect, edit, export, or activate.`}><View style={styles.grid}>{catalog.map((entry) => <View key={entry.theme.manifest.id} style={styles.gridItem}><ThemePreview themePackage={entry.theme} active={preferences.activeThemeId === entry.theme.manifest.id} onPress={() => router.push({ pathname: '/theme/[id]', params: { id: entry.theme.manifest.id } })} /><View style={styles.themeActions}><Button label={preferences.activeThemeId === entry.theme.manifest.id ? 'Active' : 'Use'} variant={preferences.activeThemeId === entry.theme.manifest.id ? 'secondary' : 'primary'} onPress={() => void runtime.selectTheme(entry.theme.manifest.id)} /><Button label="Details" variant="ghost" onPress={() => router.push({ pathname: '/theme/[id]', params: { id: entry.theme.manifest.id } })} /></View></View>)}</View></Section>
...
const styles = StyleSheet.create({ content: { padding: 14, paddingBottom: 110, gap: 24 }, grid: { gap: 14 }, gridItem: { gap: 8 }, themeActions: { flexDirection: 'row', gap: 8 }, registry: { padding: 14, gap: 10 }, input: { minHeight: 44, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, fontSize: 15 }, registryButtons: { flexDirection: 'row', gap: 8 }, market: { gap: 8 }, marketRow: { padding: 13, flexDirection: 'row', alignItems: 'center', gap: 10 }, marketCopy: { flex: 1 }, marketInfo: { padding: 15, gap: 6 } });
